...
// check argument(s)
if ($argc < 4) trigger_error("usage: " . basename($argv[0]) . " <file.dbf> <db codepage> [oem|ansi] <SID>s", E_USER_ERROR);
$db_codepage = 'CP850';
if (strtoupper($argv[2]) == 'ANSI')
 $db_codepage = 'CP1252';
$site_ids = array();
for ($i = 3; ($i + 1) <= $argc; $i++)
 array_push($site_ids, intval($argv[$i]));
...
for ($i = 0; $i < count($site_ids); $i++)
{
 ...
 {
  $record = dbase_get_record_with_names($db, $j);
  if ($record === FALSE)
  {
   dbase_close($db);
  ...
  if (intval(trim($record['SITEID'])) !== $site_ids[$i]) continue;
  $found_record = true;
  ...
  if (!dbase_replace_record($db, $record, $j))
  {
   dbase_close($db);
   var_dump($record);
   trigger_error("failed to dbase_replace_record($j), aborting\n", E_USER_ERROR);
  }
  break;
 }
 if ($found_record === false)
  ...
  echo('reset coordinates (SID was: ' .
       $site_ids[$i] .
	   ")\n");
}
dbase_close($db);
