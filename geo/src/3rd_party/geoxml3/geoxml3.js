...
geoXML3.parser = function (options) {
    ...
    if (!!doc.gpolylines) {
      for (i=0;i<doc.gpolylines.length;i++) {
        if(!!doc.gpolylines[i].infoWindow) doc.gpolylines[i].infoWindow.close();
		 doc.gpolylines[i].setVisible(false);
    ...
    if (!!doc.gpolygons) {
      for (i=0;i<doc.gpolygons.length;i++) {
        if(!!doc.gpolygons[i].infoWindow) doc.gpolygons[i].infoWindow.close();
		 doc.gpolygons[i].setVisible(false);
  ...
  var showDocument = function (doc) {
    ...
    if (!!doc.gpolylines) {
      for (i=0;i<doc.gpolylines.length;i++) {
	   doc.gpolylines[i].setVisible(true);
        // doc.gpolylines[i].setMap(parserOptions.map);
      }
    }
    if (!!doc.gpolygons) {
      for (i=0;i<doc.gpolygons.length;i++) {
 	   doc.gpolygons[i].setVisible(true);
...
var coordListA = [];
  ...
  var render = function (responseXML, doc) {
    ...
    doc.styles = styles;
      if (!!parserOptions.processStyles) {
        ...
        switch(Geometry) {
          case "Point":
			...
			if (!!window.google && !!google.maps)
             placemark.latlng = new google.maps.LatLng(placemark.Point.coordinates[0].lat,
