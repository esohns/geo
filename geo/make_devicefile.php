...
if (!$is_cli) $firephp->log(count($sites_file_contents), '#sites');
...
if (!$is_cli) $firephp->log(count($toursets_file_contents), '#toursets');
...
if (!$is_cli) $firephp->log('retrieving warehouse coordinates');
...
curl_close($curl_handle);
if (!$is_cli) $firephp->log('retrieving warehouse coordinates DONE');
...
{
 if (!$is_cli) $firephp->log($tour_ids[$i], 'processing tour');
 ...
 foreach ($toursets_file_contents as $tourset)
 {
  ...
  if (!$entry_found)
  {
   if (!$is_cli) $firephp->log($tour_ids[$i], 'invalid tour descriptor');
 ...
 if (empty($sites))
 {
  if (!$is_cli) $firephp->log($tour_ids[$i], 'invalid tour (no sites)');
 ...
 for ($j = 0; $j < count($sites); $j++)
 {
  ...
  if (!$entry_found)
  {
   if (!$is_cli) $firephp->log($sites[$j], 'invalid site ID');
