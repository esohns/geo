...
if (!file_exists($db_yields_file)) die('db yields file does not exist (was: "' .
																																							$db_yields_file .
																																							'"), aborting' . PHP_EOL);
if (!$is_cli) $firephp->log($template_file_name, 'report template');
else fwrite(STDOUT, 'report template: ' . $template_file_name . PHP_EOL);
if (!$is_cli) $firephp->log($db_areas_file, 'areas database');
...
if (!$is_cli) $firephp->log($db_yields_file, 'yields database');
...
if (!$is_cli) $firephp->log('opened areas db...');
...
for ($i = 1; $i <= $num_sites_records; $i++)
{
 ...
 {
  if (!$is_cli) $firephp->log($db_sites_record['AREAID'], strval($i) .
 ...
 if ($j == ($num_areas_records + 1))
	{
  if (!$is_cli) $firephp->log($db_sites_record['AREAID'],
...
if (!$is_cli) $firephp->log('retrieving site data...DONE');
...
if (!$is_cli) $firephp->log('opened yields db...');
...
if (!$is_cli) $firephp->log($num_yields_records, '#records (yields)');
else fwrite(STDOUT, '#records (yields): ' . $num_yields_records . PHP_EOL);
if (!$is_cli) $firephp->log('retrieving site yields...');
...
for ($i; $i <= $num_yields_records; $i++)
{
  ...
  dbase_close($db_areas);
	...
	{
  if (!$is_cli) $firephp->log($i, 'invalid record (SID was: ' .
																																		strval($db_site_id) .
																																		'), continuing');
...
if (!$is_cli) $firephp->log('retrieving site yields...DONE');
else fwrite(STDOUT, 'retrieving site yields...DONE' . PHP_EOL);
if (!$is_cli) $firephp->log('closed yields db...');
else fwrite(STDOUT, 'closed yields db...' . PHP_EOL);

if (!$is_cli) $firephp->log('grouping available areas...');
...
foreach ($sites_2_area_yields as $key => $value)
{
 ...
 if ($area_1 === 0)
	{
	 if (!$is_cli) $firephp->log($value['AREAID'], 'invalid area ID (SID was: ' .
																																																strval($key) .
																																																'), continuing');
 ...
 if ($i == ($num_areas_records + 1))
	{
	 if (!$is_cli) $firephp->log($value['AREAID'], 'invalid area (1) ID (SID was: ' .
																																																strval($key) .
																																																'), continuing');
...
for ($i = 0; $i < count($areas_1); $i++)
{
 ...
 foreach ($sites_2_area_yields as $key => $value)
 {
  ...
  if (in_array($area_2, $areas_2)) continue;
	 if ($area_2 === 0)
	 {
 	 if (!$is_cli) $firephp->log($value, 'invalid area ID (SID was: ' .
																																							strval($key) .
																																							'), continuing');
   else fwrite(STDERR, 'invalid area ID ' .
																							strval($value) .
  ...
  $areas_2[] = $area_2;
		...
		if ($j == ($num_areas_records + 1))
		{
	  if (!$is_cli) $firephp->log($value['AREAID'], 'invalid area ID (2) (SID was: ' .
																																																	strval($key) .
																																																	'), continuing');
 ...
 for ($j = 0; $j < count($areas_2); $j++)
 {
  ...
  foreach ($sites_2_area_yields as $key => $value)
  {
   ...
   $areas_3[] = $area_3;
			...
			{
	   if (!$is_cli) $firephp->log($value['AREAID'], 'invalid area (3) ID (SID was: ' .
																																																		strval($key) .
																																																		'), continuing');
...
if (!$is_cli) $firephp->log('closed areas db...');
...
for ($i = 0; $i < count($area_tree); $i++)
{
 ...
 {
	 if (!$is_cli) $firephp->log($areas_2[$j], 'processing level 2');
  ...
  {
		 if (!$is_cli) $firephp->log($areas_3[$k], 'processing level 3');
   ...
   {
			 if (!$is_cli) $firephp->log($area_id, 'processing level 4');
    ...
    if ($area_id == 0)
	   {
     if (!$is_cli) $firephp->log($sites, 'invalid area ID');
	    ...
	    continue;
				...
				for ($l = 0; $l < count($sites); $l++)
				{
				 if (!array_key_exists($sites[$l], $sites_2_area_yields))
					{
      if (!$is_cli) $firephp->log($i, 'invalid record (SID was: ' . strval($sites[$l]) . '), continuing');
      else fwrite(STDERR, 'invalid record (SID was: ' .
																										strval($sites[$l]) .
																										'), continuing' . PHP_EOL);
	     continue;
   ...
   for ($l = 0; $l < count($areas_4); $l++)
			{
			 if (!array_key_exists($areas_4[$l], $area_id_2_information))
				{
     if (!$is_cli) $firephp->log($areas_4[$l], 'invalid area ID (4)');
...
$area_tree = $area_data;
//var_dump($area_tree);
if (!$is_cli) $firephp->log('retrieving area/yield data...DONE');
else fwrite(STDOUT, 'retrieving area/yield data...DONE' . PHP_EOL);

if (!$is_cli) $firephp->log('compiling data records...');
...
if (!$is_cli) $firephp->log('compiling data records...DONE');
else fwrite(STDOUT, 'compiling data records...DONE' . PHP_EOL);

if (!$is_cli) $firephp->log('generating document...');
...
if (!$is_cli) $firephp->log($output_file, 'file name');
...
if (!$is_cli) $firephp->log('generating document...DONE');
